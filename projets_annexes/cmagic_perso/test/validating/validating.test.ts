...
beforeAll(async () => {
    ...
    const doParse = parseHelper<Model>(services.Cmagic);
    ...
    test('check capital letter validation', async () => {
        ...
        expect(
            ...
            expect.stringContaining(s`
                [1:19..1:26]: Entity name should start with a capital.
            `)
        );
    });
});
...
function checkDocumentValid(document: LangiumDocument): string | undefined {
    ...
    `
        ...
        || !isModel(document.parseResult.value) && `Root AST object is a ${document.parseResult.value.$type}, expected a '${Model.$type}'.`
        || undefined;
}

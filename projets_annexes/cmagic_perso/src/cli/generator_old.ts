...
function registerHandlebarsHelpers() {
    ...
    // Force la ré-enregistration en supprimant les helpers existants
    Handlebars.unregisterHelper('fieldConstraints');
    Handlebars.unregisterHelper('rpgFieldConstraints');
...
// La fonction generateSql reste exactement la même que précédemment.
export function generateSqlFromTemplate(model: Model): string {
 // 2. Obtenir le chemin du répertoire actuel de manière compatible ES Modules
    const __filename = fileURLToPath(import.meta.url);
    const __dirname = path.dirname(__filename);
 // 2. Charger le template depuis le disque
    const templatePath = path.join(__dirname, '../templates/schema.sql.tpl');
    const templateSource = fs.readFileSync(templatePath, 'utf-8');

    // 3. Enregistrer nos "helpers" personnalisés
    registerHandlebarsHelpers();

    // 4. Compiler le template
    const compiledTemplate = Handlebars.compile(templateSource);

    // 5. Appliquer le modèle de données (notre AST) au template
    //    Le modèle de données est un simple objet. Ici, notre AST a déjà
    //    la bonne structure { entities: [...] }

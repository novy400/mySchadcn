...
dcl-c C_NB_LIGNE_SOUSFICHIER  17;
...
//TODO: 2.  Ajouter les champs filtres de la liste.
  ...
  service like(gCTL01_t.service) inz;
//*********************************************************************
end-ds;
...
dcl-C C_CODE_DROIT 20524; // droit d'accès au module
...
dcl-proc lectSfl;
  ...
  Dow not %eof();
    ...
    clear lDetailEntity;
    sqlToEntity(lDetailDspf:lDetailEntity);
    clear lIdEntity;
    ...
    Select;
      ...
      When lSFL01.CHOIX    = '2';
        ...
        if modification(lIdEntity:lDetailEntity);
          ...
          clear lDetailDspf;
          entityToSql(lDetailEntity : lDetailDspf);
...
dcl-proc consultation;
  ...
  clear lDetailDspf;
  entityToSql(lDetail : lDetailDspf);
    ...
    select;
      ...
      When Dspf.confirm;
        ...
        if isConfirm();
          ...
          sqlToEntity(lDetailDspf:lDetail);
...
dcl-proc entityToSql;
  dcl-pi *n ;
    ...
    pDetailSql likeDs(gDspf_detail_t);
  end-pi;
  clear pDetailSql;
  eval-corr pDetailSql = pDetailEntity;  // Automatique si noms identiques
  
  // Mappings spécifiques seulement si nécessaires
  pDetailSql.code = pDetailEntity.id.code;
  pDetailSql.datembauch = pDetailEntity.dateEmbauche;  
  pDetailSql.datnaissan = pDetailEntity.dateNaissance;    
end-proc;

dcl-proc sqlToEntity;
  dcl-pi *n ;
    pDetailSql likeDs(gDspf_detail_t) const;
    pDetailEntity likeds(employee_detail_t);
  end-pi;
  clear pDetailEntity;
  eval-corr pDetailEntity = pDetailSql;  // Automatique si noms identiques
  
  // Mappings spécifiques seulement si nécessaires
  pDetailEntity.id.code = pDetailSql.code;
  pDetailEntity.dateEmbauche  = pDetailSql.datembauch;  
  pDetailEntity.dateNaissance = pDetailSql.datnaissan;    
...
dcl-proc getFiltreContext;
  ...
  if pFiltreListe <> *blanks;
    ...
    if pFiltreListe.service <> *blanks;
      lIt += 1;
      lFiltre.filter(lIt).field = 'service';
      lFiltre.filter(lIt).operator = CMAGIC_OP_EQUAL;
      lFiltre.filter(lIt).value = %trim(pFiltreListe.service);
    endif;  
  endif;

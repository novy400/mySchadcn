...
dcl-proc fournis_getlist_rest;
  ...
  if not fournis_search(lContext : lTotalCount : lItems : lErrors);
    response.contentType = IL_MEDIA_TYPE_JSON;
    CREST_addHeaders(response);
...
dcl-proc fournis_writeListResponse;
  dcl-pi *n;
    response likeDS(IL_response);
  ...
  dcl-ds lItem likeDS(fournis_item_t) based(lItemPointer);
...
dcl-proc fournis_getone_rest;
  dcl-pi *n;
    request likeDS(IL_request);
    response likeDS(IL_response);
  end-pi;
  dcl-s cId varchar(32767);
  dcl-s lId varchar(10);
  ...
  if not fournis_parseId(cId : lId);
    response.status = IL_HTTP_BAD_REQUEST;
    response.contentType = IL_MEDIA_TYPE_JSON;
    CREST_addHeaders(response);
    il_responseWrite(
      response : CREST_simpleError('Invalid Fournis id'));
    return;
  endif;

  if not fournis_get(lId : lDetail : lErrors);
    response.contentType = IL_MEDIA_TYPE_JSON;
    CREST_addHeaders(response);
  ...
  fournis_writeDetailResponse(response : lJson);
...
dcl-proc fournis_writeDetailResponse;
  dcl-pi *n;
    response likeDS(IL_response);
    pJson pointer const;
  end-pi;
...
dcl-proc fournis_registerRoutes export;
  dcl-pi *n;
    config likeDS(IL_config);
  end-pi;
  il_addRoute(config : %paddr('fournis_getlist_rest')
    : IL_GET : '^/api/fournisseurs/?$');
  il_addRoute(config : %paddr('fournis_getone_rest')
    : IL_GET : '^/api/fournisseurs/{id}$');
end-proc;

...
/include '../../includes/clientiws.rpgleinc'
...
dcl-proc test_client_iws_getlist_nominal export;
  dcl-pi *n extproc(*dclcase) end-pi;

  dcl-s items_LENGTH int(10);
  dcl-ds items likeDS(client_item_rest_t) dim(HTTPREST_MAX_ITEMS);
  dcl-ds lid likeDS(client_id_t) dim(HTTPREST_MAX_ITEMS);
...
dcl-proc test_client_iws_getlist_pagination_perPage3 export;
  dcl-pi *n extproc(*dclcase) end-pi;
...
dcl-proc test_client_iws_getlist_filter_pays_BD export;
  dcl-pi *n extproc(*dclcase) end-pi;
  ...
  rc = putenv('QUERY_STRING=codePaysNaissance=BD&perPage=100');
  ...
  for i = 1 to items_LENGTH;
    aEqual('BD' : %trim(items(i).codePaysNaissance)
         : 'Chaque element retourne doit avoir codePaysNaissance=BD');
         // check ID is valid
         lid = items(i).id;
         assert(lid.codeEtablissement = items(i).codeEtablissement : 'codeEtablissement doit etre superieur a 0');
         assert(lid.numeroClient = items(i).numeroClient : 'numeroClient doit etre superieur a 0');
         CKOOL_logMessage('code Etablissement : ' + lid.codeEtablissement );
         CKOOL_logMessage('numero Client : ' + %char(lid.numeroClient) );
  endfor;
end-proc;

dcl-proc test_client_iws_getlist_sort_nom_desc export;
  dcl-pi *n extproc(*dclcase) end-pi;
  ...
  dcl-s lExpectedFirstCode like(client_id_t.numeroClient);
  dcl-s rc int(10);

  clear lExpectedFirstCode;
  exec sql
    select numeroclient into :lExpectedFirstCode
    from client_liste
    order by nom desc
  ...
  iEqual(lExpectedFirstCode : lId.numeroClient
       : 'Le premier element doit respecter le tri nom DESC');
end-proc;

dcl-proc test_client_iws_getlist_perPage_capped_to_100 export;
  dcl-pi *n extproc(*dclcase) end-pi;
...
dcl-proc test_client_iws_getlist_not_query_string export;
  dcl-pi *n extproc(*dclcase) end-pi;
